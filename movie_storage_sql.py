...
def list_movies(user_id):
    ...
    with engine.connect() as connection:
        result = connection.execute(text("""SELECT title, year, rating, cover_url FROM movies WHERE user_id = :user_id"""),
                                    {"user_id": user_id})
        movies = result.fetchall()

    return {row[0]: {"year": row[1], "rating": row[2], "cover_url": row[3]} for row in movies}

def add_movie(title, year, rating, cover_url, user_id):
    """Add a new movie to the database."""
    with engine.connect() as connection:
        try:
            connection.execute(text("INSERT INTO movies (title, year, rating, cover_url, user_id) VALUES (:title, :year, :rating, :cover_url, :user_id)"),
                               {"title": title, "year": year, "rating": rating, "cover_url": cover_url, "user_id": user_id})
            connection.commit()
